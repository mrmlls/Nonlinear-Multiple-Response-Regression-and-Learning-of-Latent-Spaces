...
class CondRefineNetDilated(nn.Module):
    def __init__(self, image_size=32, num_classes=10, channels=3, ngf=128):
        super().__init__()
...
class Decoder(nn.Module):
    ...
    def __init__(self, indim, h, outdim):
        super().__init__()
        self.d1 = nn.Linear(outdim, h)
        self.d2 = nn.Linear(h, 2*h)
        self.dout = nn.Linear(2*h, indim)
...
#      
        dc3 = F.relu(self.d1(input))
        dc2 = F.relu(self.d2(dc3))

        output = self.dout(dc2)
...
class AE(nn.Module):
    ...
    def __init__(self, indim, h, outdim):
        super().__init__()
        self.e1 = nn.Linear(indim, 2*h)  
        self.e2 = nn.Linear(2*h, h)
        self.eout = nn.Linear(h, outdim, bias=False)
        self.d1 = nn.Linear(outdim, h)
        self.d2 = nn.Linear(h, 2*h)
        self.dout = nn.Linear(2*h, indim)
    ...
    def forward(self, input, f='c-dc'):
        if f == 'c-dc':
            ec1 = F.relu(self.e1(input))
            ec2 = F.relu(self.e2(ec1))
            ec3 = self.eout(ec2)
            dc3 = F.relu(self.d1(ec3))
            dc2 = F.relu(self.d2(dc3))
            output = self.dout(dc2)
